...
if (cd "$REPO_DIR" && PRODUCTIVE_K3S_ENGINE=unsupported ./scripts/bootstrap-k3s-stack.sh --help >/tmp/productive-k3s-invalid-engine.out 2>&1); then
...
agent_output="$(cd "$REPO_DIR" && printf '%s' "$agent_answers" | PATH="${TMP_DIR}/bin:${PATH}" PRODUCTIVE_K3S_ENGINE=k3sup ./scripts/bootstrap-k3s-stack.sh --dry-run --mode agent 2>&1)" || {
  printf '%s\n' "$agent_output" >&2
