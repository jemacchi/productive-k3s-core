# Developer Docs

This section is for repository developers and maintainers.

## Pages

### Developer references

- [Make targets for development](make-targets.md)
- [Productive K3S modes](productive-k3s-modes.md)
- [Scripts parameters](script-parameters.md)

### Guides

- [macOS development](guides/macos-development.md)
- [Post-development testing](guides/post-development-testing.md)
- [Windows development](guides/windows-development.md)

### Release and platform references

- [GitHub Actions and release automation](github-actions.md)

### Support baseline

- [Ubuntu 22.04 supported platform](ubuntu-22-04-supported.md)
- [Ubuntu 24.04 supported platform](ubuntu-24-04-supported.md)
- [Debian 12 supported platform](debian-12-supported.md)
- [Debian 13 supported platform](debian-13-supported.md)
